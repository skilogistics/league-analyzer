...
<head>
    ...
    <!-- Visualization & Content Choices: 
        ...
        - Report Info: Season data persistence. Goal: Store and retrieve data. Viz/Method: Buttons and Dropdown. Interaction: User clicks to save/load/start new seasons. Justification: Provides explicit control over data management. Library: Firebase Firestore.
    -->
    ...
    <script type="module">
        ...
        import { getFirestore, doc, setDoc, getDocs, collection, query, orderBy, limit, serverTimestamp, getDoc } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
        ...
        window.getDoc = getDoc; // Added getDoc to window object
    </script>
    <style>
...
<body class="bg-slate-50 text-slate-800">
    ...
    <main class="container mx-auto p-4 sm:p-6 lg:p-8">
        <header class="text-center mb-8">
            ...
            <p id="user-id-display" class="mt-2 text-sm text-slate-500">User ID: Loading...</p>
        </header>

        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-8">
                <section id="season-management" class="bg-white p-6 rounded-xl shadow-sm">
                    <h2 class="text-2xl font-bold mb-4 text-slate-900">Season Management</h2>
                    <p class="text-slate-600 mb-4">
                        Manage your league data by saving current progress, starting new seasons, or loading previous ones. All your data is securely stored in the cloud.
                    </p>
                    <div class="space-y-4">
                        <button id="save-season-btn" class="w-full sm:w-auto px-6 py-3 font-semibold rounded-lg btn-primary">
                            Save Current Season
                        </button>
                        <button id="new-season-btn" class="w-full sm:w-auto px-6 py-3 font-semibold rounded-lg btn-secondary">
                            Start New Season
                        </button>
                        <div>
                            <label for="season-selector" class="block mb-2 font-medium text-slate-700">Load Previous Season:</label>
                            <div class="flex flex-col sm:flex-row gap-2">
                                <select id="season-selector" class="flex-grow p-2 border rounded-md"></select>
                                <button id="load-season-btn" class="px-6 py-2 font-semibold rounded-lg btn-primary">Load Selected Season</button>
                            </div>
                        </div>
                    </div>
                </section>
    ...
    <script type="module">
        // Import Firebase globals from window (set up by the script tag above)
        const { firebaseApp, db, auth, onAuthStateChanged, signInAnonymously, signInWithCustomToken, doc, setDoc, getDocs, collection, query, orderBy, limit, serverTimestamp, getDoc } = window;
        ...
        let currentSeasonId = 'current_season'; // Special ID for the actively edited season
        let availableSeasons = []; // List of saved season metadata
        ...
        const loadSeasonBtn = document.getElementById('load-season-btn');
        ...
        function getUserSeasonsCollectionRef() {
            return collection(db, `artifacts/${appId}/users/${userId}/seasons`);
        }

        function getSeasonDocRef(seasonId) {
            return doc(db, `artifacts/${appId}/users/${userId}/seasons`, seasonId);
        }
        ...
        async function initializeApp() {
            ...
            onAuthStateChanged(auth, async (user) => {
                if (user) {
                    ...
                    await loadLatestSeason(); // Load data after auth and season list
                } else {
                    try {
                        ...
                        userIdDisplay.textContent = `User ID (Anon): ${userId}`;
                        await loadAvailableSeasons();
                        await loadLatestSeason(); // Still try to load/save with anon ID
                    }
                }
                hideLoading();
            });
        ...
        function addEventListeners() {
            ...
            // Season management event listeners
            saveSeasonBtn.addEventListener('click', saveCurrentSeasonAsNew);
            newSeasonBtn.addEventListener('click', startNewSeason);
            loadSeasonBtn.addEventListener('click', loadSelectedSeason);
        }

        async function loadAvailableSeasons() {
            try {
                const q = query(getUserSeasonsCollectionRef(), orderBy('createdAt', 'desc'));
                ...
                querySnapshot.forEach((doc) => {
                    const data = doc.data();
                    if (doc.id !== 'current_season') { // Exclude the active season from the list
                        availableSeasons.push({ id: doc.id, name: data.name || doc.id });
        ...
        async function loadLatestSeason() {
            showLoading();
            try {
                const docRef = getSeasonDocRef(currentSeasonId);
                const docSnap = await getDoc(docRef); // getDoc is now correctly imported

                if (docSnap.exists()) {
                    const data = docSnap.data();
                    results = data.results || {};
                    // teamStats are recalculated from results, so no need to load directly
                    console.log("Loaded current season data.");
                } else {
                    console.log("No current season data found. Starting fresh.");
                    results = {};
                }
                resetTeamStats();
                updateLeagueTable();
                displayFixtures(parseInt(slider.value)); // Display current day's fixtures based on loaded data
            } catch (error) {
                console.error("Error loading latest season:", error);
                // Fallback to fresh start if loading fails
                results = {};
                resetTeamStats();
                updateLeagueTable();
                displayFixtures(parseInt(slider.value));
            } finally {
                hideLoading();
            }
        }
        ...
        async function saveCurrentResults() {
            if (!userId || userId === 'loading') {
                console.warn("User not authenticated yet, cannot save results.");
                return;
            }
            ...
            // Save to Firestore
            try {
                await setDoc(getSeasonDocRef(currentSeasonId), {
                    results: results,
                    lastUpdated: serverTimestamp(),
                    name: 'Current Season' // Give current season a default name
                }, { merge: true }); // Use merge to update only the 'results' field
            } catch (error) {
                console.error("Error saving current results to Firestore:", error);
            }
        }

        async function saveCurrentSeasonAsNew() {
            if (!userId || userId === 'loading') {
                console.warn("User not authenticated yet, cannot save season.");
                return;
            }
            ...
            showLoading();
            try {
                // First, ensure current results are saved to the 'current_season' document
                await saveCurrentResults(); 
                
                // Then, copy the current_season data to a new season document
                const currentSeasonDoc = await getDoc(getSeasonDocRef(currentSeasonId));
                if (currentSeasonDoc.exists()) {
                    const dataToSave = currentSeasonDoc.data();
                    dataToSave.name = seasonName;
                    dataToSave.createdAt = serverTimestamp(); // Add a timestamp for sorting
                    
                    const newSeasonId = `${seasonName.replace(/\s+/g, '_')}_${Date.now()}`; // Unique ID
                    await setDoc(getSeasonDocRef(newSeasonId), dataToSave);
                    console.log(`Season "${seasonName}" saved successfully!`);
                    await loadAvailableSeasons(); // Refresh the dropdown
                } else {
                    console.warn("No current season data to save.");
                }
            } catch (error) {
        ...
        async function startNewSeason() {
            if (!userId || userId === 'loading') {
                console.warn("User not authenticated yet, cannot start new season.");
                return;
            }
            if (!confirm("Are you sure you want to start a new season? All unsaved current progress will be lost unless you save it first.")) {
                return;
            }
            ...
            try {
                ...
                results = {};
                resetTeamStats();
                slider.value = 1; // Reset slider to day 1
                matchdayLabel.textContent = 1;
                fixturesDayLabel.textContent = 1;

                // Clear 'current_season' in Firestore
                await setDoc(getSeasonDocRef(currentSeasonId), {
                    results: {},
                    name: 'Current Season',
                    lastUpdated: serverTimestamp()
        ...
        async function loadSelectedSeason() {
            if (!userId || userId === 'loading') {
                console.warn("User not authenticated yet, cannot load season.");
                return;
            }
            const selectedSeasonId = seasonSelector.value;
            if (!selectedSeasonId) {
                alert("Please select a season to load.");
                return;
            }

            showLoading();
            try {
                const docRef = getSeasonDocRef(selectedSeasonId);
                const docSnap = await getDoc(docRef); // getDoc is now correctly imported

                if (docSnap.exists()) {
                    const data = docSnap.data();
                    results = data.results || {};
                    // Reset slider and display fixtures for day 1 of loaded season
                    slider.value = 1;
                    matchdayLabel.textContent = 1;
                    fixturesDayLabel.textContent = 1;
                    ...
                    updateLeagueTable();
                    displayFixtures(1);
                    console.log(`Season "${data.name || selectedSeasonId}" loaded successfully!`);
                } else {
                    console.warn(`Season "${selectedSeasonId}" not found.`);
                }
            } catch (error) {
